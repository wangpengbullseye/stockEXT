...
async function findNextKlineFromData(lastKline, period) {
    ...
    try {
        ...
        if (minuteData && minuteData.length > 0) {
            ...
            // 从分钟数据中聚合出下一根K线
            const nextKline = aggregateNextKlineFromMinuteData(minuteData, period, lastKline);
...
// 聚合下一根小时K线
function aggregateNextHourlyKline(minuteData, lastTime, lastDateOnly) {
    ...
    const endIndex = startIndex + 60; // 60分钟 = 1小时
        ...
        try {
            ...
            if (this.previewSeries) {
                ...
                if (point1 && point2) {
                    ...
                    // 保存当前视图状态（在计算前保存）
                    const currentVisibleRange = this.chart.timeScale().getVisibleRange();

                    // 向右：创建射线预览（使用完整的边界交点计算，带日志）
                    ...
                    if (this.validateLineData(previewData)) {
                        ...
                        try {
                            if (currentVisibleRange) {
                                this.chart.timeScale().setVisibleRange(currentVisibleRange);
                            }
                        } catch (error) {
                            // 忽略恢复失败
                        }
    ...
    createDrawing() {
        ...
        try {
            ...
            if (!this.userAdjustedView) {
                const restoreView = () => {
                    try {
                        ...
                        if (currentLogicalRange) {
                            this.chart.timeScale().setVisibleLogicalRange(currentLogicalRange);
                        }
                    } catch (error) {
                        // 忽略错误
                    }
                ...
                // 使用requestAnimationFrame在下一帧再次恢复
                ...
                console.log('🔒 已设置视图保护（用户未手动调整）');
            } else {
                console.log('🎯 用户已手动调整视图，跳过自动恢复');
            }
...
class DrawingToolSystem {
    ...
    checkDrawingProximity(param) {
        ...
        // 防止过度调用，限制检测频率
        const now = Date.now();
        if (this.lastProximityCheck && now - this.lastProximityCheck < 50) {
            return; // 限制为每50ms检测一次
        }
        ...
        for (const drawing of drawings) {
            try {
                const distance = this.getDistanceToDrawing(drawing, param);
    ...
    getDistanceToDrawing(drawing, param) {
        try {
            ...
            switch (drawing.type) {
                case 'horizontalLine':
                    // 水平线距离计算
                    const lineY = this.candleSeries.priceToCoordinate(drawing.price);
                ...
                case 'trendLine':
                    ...
                    // 对于射线，使用实际绘制的数据来计算距离
                    if (drawing.data && drawing.data.length >= 2) {
                        const startPoint = { time: drawing.data[0].time, price: drawing.data[0].value };
                        const endPoint = { time: drawing.data[drawing.data.length - 1].time, price: drawing.data[drawing.data.length - 1].value };
                        return this.getDistanceToLine(startPoint, endPoint, param, timeScale);
                    }
                    return this.getDistanceToLine(drawing.points[0], drawing.points[1], param, timeScale);

                case 'verticalLine':
                    // 垂直线距离计算
                    const lineX = timeScale.timeToCoordinate(drawing.time);
                    if (lineX === null) return Infinity;
                    return Math.abs(param.point.x - lineX);

                case 'rectangle':
                    // 矩形距离计算（到边框的距离）
                    if (!drawing.points || drawing.points.length < 2) return Infinity;
                    return this.getDistanceToRectangle(drawing.points[0], drawing.points[1], param, timeScale);
    ...
    getDistanceToLine(point1, point2, mouseParam, timeScale) {
        try {
            ...
            // 安全的坐标转换
            try {
                x1 = timeScale.timeToCoordinate(point1.time);
                y1 = this.candleSeries.priceToCoordinate(point1.price);
                x2 = timeScale.timeToCoordinate(point2.time);
                y2 = this.candleSeries.priceToCoordinate(point2.price);
            } catch (coordError) {
                console.warn('⚠️ 坐标转换失败:', coordError.message);
                return Infinity;
            }
            ...
            console.warn('⚠️ 计算线段距离失败:', error.message);
            return Infinity;
        }
    }
    ...
    getDistanceToRectangle(point1, point2, mouseParam, timeScale) {
        try {
            ...
            const y1 = this.candleSeries.priceToCoordinate(Math.max(point1.price, point2.price));
            const x2 = timeScale.timeToCoordinate(Math.max(point1.time, point2.time));
            const y2 = this.candleSeries.priceToCoordinate(Math.min(point1.price, point2.price));
